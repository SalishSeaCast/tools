...
def load_TS(filename):
    ...
    date = pd.DataFrame(
        {
            "date": [datetime_start + timedelta(hours=hour) for hour in data["time"]],
        }
    )
...
def load_hoff(filename):
    ...
    hoff_out = xr.concat(
        data_list,
        dim=xr.DataArray(datetime_index, name="time", dims="time"),
    )
...
def loadSOG(filepath):
    ...
    pH_sur, Omega_A_sur = carb.calc_carbonate(
        ...
        calc_sigma(
            phys_TS["surface temperature"], phys_TS["surface salinity"]
        ),  # sigma_t [kg m3]
        ...
        bio_TS["surface silicon concentration"],
    )  # silicate [uM]
    ...
    pH_3m, Omega_A_3m = carb.calc_carbonate(
        ...
        calc_sigma(
            phys_TS["3 m avg temperature"], phys_TS["3 m avg salinity"]
        ),  # sigma_t [kg m3]
        ...
        bio_TS["3 m avg silicon concentration"],
    )  # silicate [uM]
