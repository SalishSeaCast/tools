...
def read_river_Theodosia(config):
    part1 = pd.read_csv(config["rivers"]["SOG river files"]["TheodosiaScotty"], header=None, sep='\s+', index_col=False,
                      names=['year', 'month', 'day', 'flow'])
    part2 = pd.read_csv(config["rivers"]["SOG river files"]["TheodosiaBypass"], header=None, sep='\s+', index_col=False,
                      names=['year', 'month', 'day', 'flow'])
    part3 = pd.read_csv(config["rivers"]["SOG river files"]["TheodosiaDiversion"], header=None, sep='\s+', index_col=False,
                      names=['year', 'month', 'day', 'flow'])
    ...
    while notfitted:
        ...
        if fittype == 'persist':
            ...
            if fittype == 'fit':
                ...
                stop
            bad, flux = patch_fitting(primary_river, useriver, dateneeded, gap_length, config)
            if bad:
                method = method + 1
            else:
                notfitted = False
...
def do_a_pair(water_shed, watershed_from_river, dateneeded,
              primary_river_name, use_secondary, config,
    ...
    if use_secondary:
        ...
        if len(secondary_river[secondary_river.index == str(dateneeded.date())]) == 1:
            ...
            secondary_flow = patch_gaps(secondary_river_name, secondary_river, dateneeded, config)
...
def do_fraser(watershed_from_river, dateneeded,
              primary_river_name, secondary_river_name, config):
    primary_river = read_river(primary_river_name, 'primary', config)
