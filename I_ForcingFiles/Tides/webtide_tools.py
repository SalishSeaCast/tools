...
def get_data_from_csv(tidevar, constituent, depth):
    ...
    #correction factors
    pha_K1 = 1.61#K1 phase correction in degrees.    applied to velocity and ssh
    amp_K1 = 0.921 #K1 amp correction factor      applied to velocity and ssh
    pha_M2 = 22 #M2 phase correction in degrees     applied to velocity and ssh
...
def create_northern_tides_contd(Z1,Z2,tidevar,constituent):
    import netCDF4 as NC
    import numpy as np
    from salishsea_tools import nc_tools
    
    nemo = NC.Dataset('SalishSea2_corr_North_tide_'+constituent+'_grid_'+tidevar+'.nc', 'w', zlib=True)
