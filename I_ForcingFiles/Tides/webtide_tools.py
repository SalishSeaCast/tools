...
def get_data_from_csv(
    ...
    Vfilename="Tidal Current Constituents V.csv",
    inorth=0,
):
    ...
    if tidevar == "T":
        webtide = pd.read_csv(Tfilename, skiprows=2)
        webtide = webtide.rename(
            columns={
                "Constituent": "const",
                "Longitude": "lon",
                "Latitude": "lat",
        ...
        nwebtide = int(webtide.shape[0] / 8.0)
        ...
        amp_W[gap : boundlen - inorth, 0] = (
            webtide[webtide.const == (base + ":")].amp * corr_amp
        )
        amp_W[0:gap, 0] = amp_W[gap, 0]
        amp_W[boundlen - inorth :, 0] = amp_W[boundlen - inorth - 1, 0]

        pha_W[gap : boundlen - inorth, 0] = (
            webtide[webtide.const == (base + ":")].pha + corr_pha
        )
    ...
    # U VELOCITY
    if tidevar == "U":
        webtide = pd.read_csv(Ufilename, skiprows=2)
        webtide = webtide.rename(
            columns={
                "Constituent": "const",
                "Longitude": "lon",
        ...
        uphi_ugrid = numpy.radians(
            numpy.array(webtide[webtide.const == (base + ":")].ewpha)
        )
        vphi_ugrid = numpy.radians(
            numpy.array(webtide[webtide.const == (base + ":")].nspha)
        )

        uZ1 = ua_ugrid * numpy.cos(theta) * numpy.cos(
            uphi_ugrid
        ) - va_ugrid * numpy.sin(theta) * numpy.sin(vphi_ugrid)
        uZ2 = ua_ugrid * numpy.cos(theta) * numpy.sin(
            uphi_ugrid
        ...
        for i in range(0, len(amp)):
            pha.append(
                math.atan2(uZ2[i], uZ1[i]) + numpy.radians(corr_pha + corr_shift)
            )
    ...
    # V VELOCITY
    if tidevar == "V":
        webtide = pd.read_csv(Vfilename, skiprows=2)
        webtide = webtide.rename(
            columns={
                "Constituent": "const",
                "Longitude": "lon",
        ...
        uphi_vgrid = numpy.radians(
            numpy.array(webtide[webtide.const == (base + ":")].ewpha)
        )
        vphi_vgrid = numpy.radians(
            numpy.array(webtide[webtide.const == (base + ":")].nspha)
        )

        vZ1 = -ua_vgrid * numpy.sin(theta) * numpy.cos(
            uphi_vgrid
        ) - va_vgrid * numpy.cos(theta) * numpy.sin(vphi_vgrid)
        vZ2 = -ua_vgrid * numpy.sin(theta) * numpy.sin(
            uphi_vgrid
        ) + va_vgrid * numpy.cos(theta) * numpy.cos(vphi_vgrid)
        ...
        for i in range(0, len(amp)):
            pha.append(
                math.atan2(vZ2[i], vZ1[i]) + numpy.radians(corr_pha + corr_shift)
            )
...
def create_northern_tides(Z1, Z2, tidevar, constituent, code, name="SalishSea2"):
    ...
    nemo = NC.Dataset(
        name + "_" + code + "_North_tide_" + constituent + "_grid_" + tidevar + ".nc",
        "w",
        zlib=True,
    )
    ...
    nc_tools.init_dataset_attrs(
        ...
        nc_filepath="../../../nemo-forcing/open_boundaries/north/SalishSea2_North_tide_"
        + constituent
        + "_grid_"
        + tidevar
        + ".nc",
...
def create_northern_tides_contd(Z1, Z2, tidevar, constituent, code, name="SalishSea2"):
    ...
    nc_tools.init_dataset_attrs(
        ...
        nc_filepath="../../../NEMO-forcing/open_boundaries/north/tides/SalishSea2_North_tide_"
        + constituent
        + "_grid_"
        + tidevar
        + ".nc",
