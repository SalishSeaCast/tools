...
def reshape_BCs(infile, outfile):
    ...
    for k, v in fin.variables.items():
        ...
        # Create variables
        fout.createVariable(
            v.name, v.datatype, v.dimensions, zlib=True, complevel=4, shuffle=False
        )
    ...
    # Update notes
    fout.source += (
        "\n https://github.com/SalishSeaCast/" "tools/I_ForcingFiles/OBC/reshape_BCs.py"
    )

    fout.history += (
        "\n [{}] Reshaped to structured format and drop "
        "variables nbidta, nbjdta, and nbrdta, with compression.".format(
