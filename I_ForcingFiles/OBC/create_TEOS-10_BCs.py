...
def create_BCs(infile, outfile, title, dS_is_zero=True):
    ...
    dep = np.expand_dims(
        np.expand_dims(np.expand_dims(F.variables["deptht"][:], axis=0), axis=2), axis=3
    ) + np.zeros(sal.shape)
    ...
    if dS_is_zero:
        ...
        sal_abs = gsw_calls.generic_gsw_caller(
            "gsw_SA_from_SP.m", [sal_pract, p, long, lat]
        )
        sal_title = "Absolute Salinity"
    temp_pot = np.copy(temp[:])
    temp_cons = gsw_calls.generic_gsw_caller("gsw_CT_from_pt.m", [sal_abs, temp_pot])
    # Write into netcdf file
    sal[:] = sal_abs
    temp[:] = temp_cons
    # Update variable metadata
    sal.setncatts({"units": "g/kg", "long_name": sal_title})
    temp.setncatts({"units": "deg C", "long_name": "Conservative Temperature"})
    ...
    F.comment = (
        "Temperature and salinity are TEOS-10 variables:"
        " Conservative Temperature and {}".format(sal_title)
    )

    F.references = (
        "https://github.com/SalishSeaCast/nemo-forcing/src/tip/"
        "open_boundaries/{}/{}".format(
            os.path.basename(os.path.split(outfile)[0]), os.path.basename(outfile)
        )
    )
    ...
    F.history = history + (
        ...
        "Temperature and "
        "{}".format(datetime.datetime.today().strftime("%Y-%m-%d"), sal_title)
    )
