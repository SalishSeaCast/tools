...
def get_EC_observations(station, start_day, end_day):
    ...
    for record in raw_data:
        ...
        selectors = day >= st_ar.day and day <= end_ar.day
        if selectors:
            try:
...
def compile_model_output(i, j, files, model):
    ...
    for f in files:
        ...
        pr.append(G.variables["atmpres"][:, j, i])
        sol.append(G.variables["solar"][:, j, i])
        qr.append(G.variables["qair"][:, j, i])
        the.append(G.variables["therm_rad"][:, j, i])
    ...
    wind = np.array(wind).reshape(
        len(filesGEM) * 24,
    )
    direc = np.array(direc, "double").reshape(
        len(filesGEM) * 24,
    )
    t = np.array(t).reshape(
        len(filesGEM) * 24,
    )
    pr = np.array(pr).reshape(
        len(filesGEM) * 24,
    )
    tem = np.array(tem).reshape(
        len(filesGEM) * 24,
    )
    sol = np.array(sol).reshape(
        len(filesGEM) * 24,
    )
    the = np.array(the).reshape(
        len(filesGEM) * 24,
    )
    qr = np.array(qr).reshape(
        len(filesGEM) * 24,
    )
    pre = np.array(pre).reshape(
        len(filesGEM) * 24,
    )
...
def compile_model_output_MF(i, j, files, model):
    ...
    u = G.variables["u_wind"][:, j, i]
    v = G.variables["v_wind"][:, j, i]
    pr = G.variables["atmpres"][:, j, i]
    sol = G.variables["solar"][:, j, i]
    qr = G.variables["qair"][:, j, i]
    the = G.variables["therm_rad"][:, j, i]
    pre = G.variables["precip"][:, j, i]
    tem = G.variables["tair"][:, j, i]
...
def gather_observed_winds(location, startdate, numdays):
    ...
    dates.sort()
    strd = dates[0].astimezone(pytz.timezone("US/Pacific"))
    start = strd.strftime("%d-%b-%Y")
    ...
    if strd.month == dates[-1].astimezone(pytz.timezone("US/Pacific")).month:
        ...
        [winds, dirs, temps, times, lat, lon] = get_EC_observations(
            location, start, end
        )
    ...
    # if spans more than one month, we have to be more careful
        ...
        while m != dates[-1].astimezone(pytz.timezone("US/Pacific")).month:
            ...
            if d.month != m:
                ...
                [wind_speed, wind_dir, temp, time, lat, lon] = get_EC_observations(
                    location, start, end
                )
                ...
                temps.append(temp)
                m = d.month
                start = d.strftime("%d-%b-%Y")
                count = count + 1
            # after the loop
        endd = dates[-1].astimezone(pytz.timezone("US/Pacific"))
        end = endd.strftime("%d-%b-%Y")
        [wind_speed, wind_dir, temp, time, lat, lon] = get_EC_observations(
            location, start, end
        )
        winds.append(wind_speed)
        winds = _flatten_list(winds)
        winds = np.array(winds)
        dirs.append(wind_dir)
        dirs = _flatten_list(dirs)
        dirs = np.array(dirs)
        times.append(time)
        times = _flatten_list(times)
        times = np.array(times)
        temps.append(temp)
        temps = _flatten_list(temps)
    ...
    winds = winds[
        np.where(
            (times < (startdate + datetime.timedelta(days=1)))
