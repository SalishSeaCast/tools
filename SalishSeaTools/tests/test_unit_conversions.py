...
def test_mps_kph(m_per_s, expected):
    np.testing.assert_allclose(unit_conversions.mps_kph(m_per_s), expected)
...
def test_knots_mps(knots, expected):
    np.testing.assert_allclose(unit_conversions.knots_mps(knots), expected, rtol=1e-05)
...
def test_wind_to_from(wind_to, expected):
    np.testing.assert_allclose(unit_conversions.wind_to_from(wind_to), expected)


def test_wind_to_from_ndarray():
    wind_from = unit_conversions.wind_to_from(np.array([0, 90, 180, 270, 359]))
...
class TestBearingHeading(object):
    ...
    def test_8_points(self, bearing, expected):
        heading = unit_conversions.bearing_heading(
            bearing, headings=["N", "NE", "E", "SE", "S", "SW", "W", "NW", "N"]
        )
        assert heading == expected
