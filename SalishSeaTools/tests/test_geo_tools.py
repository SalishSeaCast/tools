...
class TestDistanceAlongCurve:
    ...
    @pytest.mark.parametrize(
        "lons, lats, expected",
        [
            (np.array([0, 0]), np.array([0, 1]), np.array([0, 60 * KM_PER_NM])),
            (
                np.array([-123, -123, -123.5]),
                np.array([49, 50, 50.5]),
...
class TestFindClosestModelPoint:
    ...
    model_lons = np.array(
        [
            [-124.5007019, -124.49552155, -124.49034119, -124.4851532, -124.47997284],
            [-124.50404358, -124.49885559, -124.49367523, -124.48848724, -124.48330688],
            [-124.50737762, -124.50219727, -124.49700928, -124.49182892, -124.48664093],
            [-124.5107193, -124.50553131, -124.50035095, -124.49516296, -124.4899826],
        ]
    )
    ...
    def test_raises_value_error(self):
        with pytest.raises(ValueError):
            ...
            geo_tools.find_closest_model_point(
                0, 0, self.model_lons, self.model_lats, raiseOutOfBounds=True
            )

    @pytest.mark.parametrize(
        "lon, lat, expected",
        [(-124.488, 48.54, (0, 2)), (-124.5, 48.54, (1, 0)), (-124.5, 48.555, (1, 0))],
    )
