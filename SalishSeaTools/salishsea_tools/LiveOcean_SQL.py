        ...
        for k in range(data.shape[1]):
            ...
            try:
                filled[t, k, mask] = interpolate.griddata(
                    points, valid_data, (lons[mask], lats[mask]), method="nearest"
                )
            except ValueError:
...
def interpolate_to_NEMO_lateral(var_arrays, dataset, NEMOlon, NEMOlat, shape):
    ...
    for var_name, var in var_arrays.items():
        ...
        for t in range(var_new.shape[0]):
            for k in range(var_new.shape[1]):
                ...
                var_interp = Basemap.interp(var_grid, lonsLO, latsLO, NEMOlon, NEMOlat)
                # Keep track of mask
    ...
    for k in range(1, var_new.shape[1]):
        interps["salt"][:, k] = np.fmax(
            interps["salt"][:, k], interps["salt"][:, k - 1]
        )
...
def _increasing_density(filled):
    ...
    while not stable:
        for t in np.arange(filled["salt"].shape[0]):
            approx_rho_stable = beta * (
                filled["salt"][t, 1:] - filled["salt"][t, :-1]
            ) - alpha * (filled["temp"][t, 1:] - filled["temp"][t, :-1])
...
def _bioFileSetup(TS, new):
    ...
    deptht.valid_range = np.array((4.0, 428.0))
    ...
    # NO3
    voNO3 = new.createVariable(
        "NO3", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    ...
    # Si
    voSi = new.createVariable("Si", "float32", ("time_counter", "deptht", "yb", "xbT"))
...
def _ginterp(xval, xPeriod, yval, L, xlocs):
    ...
    for ii in range(0, xlocs.size):
        t = xlocs[ii]
        diff = [
            min(abs(x - t), abs(x - t + xPeriod), abs(x - t - xPeriod)) for x in xval
        ]
        weight = [
            np.exp(-0.5 * x**2 / s**2) if sum(diff < x) < 2 or x < 5 else 0.0
            for x in diff
        ]
...
def _ginterp2d(xval, xPeriod, yval, yPeriod, zval, L, M, zlocs_x, zlocs_y):
    ...
    for ii in range(0, zlocs_x.size):
        for jj in range(0, zlocs_y.size):
            ...
            ty = zlocs_y[jj]
            diffx = [
                min(abs(x - tx), abs(x - tx + xPeriod), abs(x - tx - xPeriod))
                for x in xval
            ]
            diffy = [
                min(abs(y - ty), abs(y - ty + yPeriod), abs(y - ty - yPeriod))
                for y in yval
            ]
            weight = [
                (
                    np.exp(-0.5 * (x**2 + y**2) / (s**2 + n**2))
                    if (sum(diffx < x) < 3 or x < L) and (sum(diffy < y) < 3 or y < M)
                    else 0.0
                )
...
def recalcBioTSFits(
    ...
    try:
        ...
        raise ImportError(
            "You need to install sqlalchemy in your environment to run recalcBioTSFits."
        )
    # Load 3D T+S
    ...
    # NH4
    voNH4 = newConst.createVariable(
        "NH4", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    ...
    # don't yet set values
    # DIA
    voDIA = newConst.createVariable(
        "DIA", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    voDIA.units = "muM N"
    ...
    voDIA[:] = val_DIA
    # CRY
    voCRY = newConst.createVariable(
        "CRY", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    voCRY.units = "muM N"
    ...
    voCRY[:] = val_CRY
    # MYRI
    voMYRI = newConst.createVariable(
        "MYRI", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    voMYRI.units = "muM N"
    ...
    voMYRI[:] = val_MYRI
    # MICZ
    voMICZ = newConst.createVariable(
        "MICZ", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    voMICZ.units = "muM N"
    ...
    voMICZ[:] = val_MICZ
    # PON
    voPON = newConst.createVariable(
        "PON", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    voPON.units = "muM N"
    ...
    # voPON[:] = val_PON
    # DON
    voDON = newConst.createVariable(
        "DON", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    voDON.units = "muM N"
    ...
    # voDON[:]=DON_val
    # bSi
    vobSi = newConst.createVariable(
        "bSi", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    vobSi.units = "muM N"
    ...
    vobSi[:] = val_bSi
    # O2
    voO2 = newConst.createVariable(
        "O2", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    # voO2.units = ''
    ...
    voO2[:] = val_Oxy
    # turbidity
    votu = newConst.createVariable(
        "tur", "float32", ("time_counter", "deptht", "yb", "xbT")
    )
    # voO2.units = ''
    ...
    # definitions
    SA = case(
        [(Calcs.Salinity_Bottle_SA != None, Calcs.Salinity_Bottle_SA)],
        else_=case(
            [(Calcs.Salinity_T0_C0_SA != None, Calcs.Salinity_T0_C0_SA)],
            else_=case(
                [(Calcs.Salinity_T1_C1_SA != None, Calcs.Salinity_T1_C1_SA)],
                else_=case(
                    [(Calcs.Salinity_SA != None, Calcs.Salinity_SA)],
                    else_=case(
                        [
                            (
                                Calcs.Salinity__Unknown_SA != None,
                                Calcs.Salinity__Unknown_SA,
                            )
                        ],
                        else_=Calcs.Salinity__Pre1978_SA,
                    ),
                ),
            ),
        ),
    ...
    NO = case(
        [(Obs.Nitrate_plus_Nitrite != None, Obs.Nitrate_plus_Nitrite)],
        else_=Obs.Nitrate,
    )
    NOUnits = case(
        [(Obs.Nitrate_plus_Nitrite != None, Obs.Nitrate_plus_Nitrite_units)],
        else_=Obs.Nitrate_units,
    )
    NOFlag = case(
        [(Obs.Nitrate_plus_Nitrite != None, Obs.Flag_Nitrate_plus_Nitrite)],
        else_=Obs.Flag_Nitrate,
    )
    # Obs.Quality_Flag_Nitr does not match any nitrate obs
    # ISUS not included in this NO
    Tem = case(
        [(Obs.Temperature != None, Obs.Temperature)],
        else_=case(
            [(Obs.Temperature_Primary != None, Obs.Temperature_Primary)],
            else_=case(
                [(Obs.Temperature_Secondary != None, Obs.Temperature_Secondary)],
                else_=Obs.Temperature_Reversing,
            ),
        ),
    )
    TemUnits = case(
        [(Obs.Temperature != None, Obs.Temperature_units)],
        else_=case(
            [(Obs.Temperature_Primary != None, Obs.Temperature_Primary_units)],
            else_=case(
                [(Obs.Temperature_Secondary != None, Obs.Temperature_Secondary_units)],
    ...
    Ox = case(
        [(Calcs.Oxygen_umolL != None, Calcs.Oxygen_umolL)],
        else_=Calcs.Oxygen_Dissolved_umolL,
    )
    OxFlag = case(
        [(Calcs.Oxygen_umolL != None, Obs.Quality_Flag_Oxyg)],
        else_=Obs.Flag_Oxygen_Dissolved,
    )
    ...
    q = (
        ...
        .select_from(Obs)
        .join(JDFLocs, JDFLocs.ObsID == Obs.ID)
        .join(Station, Station.ID == Obs.StationTBLID)
        .join(Calcs, Calcs.ObsID == Obs.ID)
        .filter(Obs.Ammonium != None)
        .all()
    )
    qP = []
    qNH = []
    ...
    q = (
        ...
        .select_from(Obs)
        .join(Station, Station.ID == Obs.StationTBLID)
        .filter(Obs.Nitrogen_Dissolved_Organic != None)
        .filter(Obs.Nitrogen_Dissolved_Organic >= 0)
        .filter(Station.Lat != None)
        .filter(Station.Lon != None)
        .filter(Station.Lat < 48.8)
        .filter(Station.Lon < -125)
        .all()
    )
    ...
    q = (
        session.query(
            ...
            Station.Lon,
            Obs.Depth,
            Obs.Nitrogen_Particulate_Organic,
            Obs.Nitrogen_Particulate_Organic_units,
            Tem,
        )
        .select_from(Obs)
        .join(Station, Station.ID == Obs.StationTBLID)
        .filter(Obs.Nitrogen_Particulate_Organic != None)
        .filter(Obs.Nitrogen_Particulate_Organic >= 0)
        .filter(Station.Lat != None)
        .filter(Station.Lon != None)
        .filter(Station.Lat < 48.8)
        .filter(Station.Lon < -125)
...
# ------------------ Creation of files ------------------------------
def create_LiveOcean_bio_BCs_fromTS(
    TSfile,
    strdate=None,
    TSdir="/results/forcing/LiveOcean/boundary_conditions",
    ...
    if recalcFits == True:
        recalcBioTSFits(
            nFitFilePath=nFitFilePath,
            siFitFilePath=siFitFilePath,
            nClimFilePath=nClimFilePath,
            siClimFilePath=siClimFilePath,
        )
    ...
    # process N
    ztan = [0.5 * math.tanh((a - 70) / 20) + 1 / 2 for a in zupper]
    zcoeff = np.ones(
        np.shape(TS.variables["votemper"])
    )  # zcoeff is multiplier of fit function; 1-zcoeff is multiplier of climatology
    for i in range(0, zupper.size):
        zcoeff[:, i, :, :] = ztan[i]
    funfit = (
        mC
        + mT * TS.variables["votemper"][:, :, :, :]
        + mS * TS.variables["vosaline"][:, :, :, :]
    )
...
def create_LiveOcean_TS_BCs(
    ...
    LO_to_NEMO_var_map = {"salt": "vosaline", "temp": "votemper"}
    ...
    lateral_interps = interpolate_to_NEMO_lateral(
        depth_interps, LO_dataset, lonBC, latBC, shape
    )
    ...
    # move files around
    if nowcast:
        filepaths = _relocate_files_for_nowcast(start, save_dir, basename, bc_dir)
    elif single_nowcast:
        filepaths = []
        d_file = os.path.join(
...
def _relocate_files_for_nowcast(start_date, save_dir, basename, bc_dir):
    ...
    for d, subdir in zip([1, 2], ["", "fcst"]):
        ...
        d_file = os.path.join(
            save_dir, "{}_{}.nc".format(basename, next_date.strftime("y%Ym%md%d"))
        )
...
def _separate_and_save_files(
    ...
    for counter, t in enumerate(interpolated_data["ocean_time"]):
        ...
        filenames = {
            "yearly": os.path.join(save_dir, "{}_y{}.nc".format(basename, first.year)),
            "monthly": os.path.join(
                save_dir, "{}_y{}m{:02d}.nc".format(basename, first.year, first.month)
            ),
            "daily": os.path.join(
                save_dir,
                "{}_y{}m{:02d}d{:02d}.nc".format(
                    basename, first.year, first.month, first.day
                ),
            ),
        }
        if conditions[file_frequency]:
            for LO_name, NEMO_name in LO_to_NEMO_var_map.items():
                NEMO_var_arrays[NEMO_name] = interpolated_data[LO_name][
                    index:counter, :, :, :
                ]
            _create_sub_file(
                first,
                time_units[avg_period],
                NEMO_var_arrays,
                var_meta,
                NEMO_BC_file,
...
def _create_sub_file(date, time_unit, var_arrays, var_meta, NEMO_BC, filename):
    ...
    ds["time_counter"].attrs["units"] = "{} since {}".format(
        time_unit, date.strftime("%Y-%m-%d %H:%M:%S")
    )
    ...
    ds.attrs = {
        ...
        "summary": (
            "Temperature and Salinity from the Live Ocean model"
...
def _convert_TS_to_TEOS10(var_meta, sal, temp):
    ...
    sal_ref = gsw_calls.generic_gsw_caller(
        "gsw_SR_from_SP.m",
        [
            sal[:],
        ],
