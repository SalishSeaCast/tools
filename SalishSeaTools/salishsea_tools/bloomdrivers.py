...
def getWindVarsYear(year, loc):
    ...
    if year > 2014:
        ...
        with xr.open_dataset(
            "/results/forcing/atmospheric/GEM2.5/gemlam/gemlam_y2012m03d01.nc"
        ) as gridrefWind:
            # always use a post-2011 file here to identify station grid location
            lon, lat = places.PLACES[loc]["lon lat"]
            jW, iW = geo_tools.find_closest_model_point(
                lon,
                lat,
                gridrefWind.variables["nav_lon"][:, :] - 360,
                gridrefWind.variables["nav_lat"][:, :],
                grid="GEM2.5",
            )
...
def metric1_bloomtime(phyto_alld, no3_alld, bio_time):
    ...
    for i, row in metric1_df.iterrows():
        try:
            if (
                metric1_df["upper_3m_no3"].iloc[i] < 2
                and metric1_df["upper_3m_no3"].iloc[i + 1] < 2
            ):
                location1 = i
                break
        except IndexError:
...
def metric2_bloomtime(phyto_alld, no3_alld, bio_time):
    ...
    df["phytopeaks"] = df.sphyto[
        (df.sphyto.shift(1) < df.sphyto) & (df.sphyto.shift(-1) < df.sphyto)
    ]
    ...
    for i, row in df.iterrows():
        try:
            if (
                df["sphyto"].iloc[i - 1] > chlvalue
                and df["sphyto"].iloc[i - 2] > chlvalue
                and pd.notna(df["phytopeaks"].iloc[i])
            ):
...
def metric3_bloomtime(sphyto, sno3, bio_time):
    ...
    for i, row in weeklychl.iterrows():
        try:
            if (
                weeklychl["sphyto"].iloc[i] > threshold
                and weeklychl["sphyto"].iloc[i + 1] > secondthresh
            ):
                bloomtime3 = weeklychl.bio_time[i]
                break
            elif (
                weeklychl["sphyto"].iloc[i] > threshold
                and weeklychl["sphyto"].iloc[i + 2] > secondthresh
            ):
                bloomtime3 = weeklychl.bio_time[i]
                break
        except IndexError:
...
def halo_de(ncname, ts_x, ts_y):
    ...
    if len(col_depths) == 0:
        ...
        for i in range(0, (len(col_sal) - 1)):
            sal_grad[i] = np.abs(
                (col_sal[i] - col_sal[i + 1]) / (col_depths[i] - col_depths[i + 1])
            )
        ...
        halocline = col_depths[loc_max] + 0.5 * (
            col_depths[loc_max + 1] - col_depths[loc_max]
        )
...
def density_diff(sal, temp, time):
    ...
    for ind, depth in depthrange.items():
        ...
        surfacedens = gsw.rho(
            dsal.iloc[:, 0], dtemp.iloc[:, 0], p
        )  # get the surface density
        idens = gsw.rho(
            dsal.iloc[:, ind], dtemp.iloc[:, ind], p
        )  # get the density at that depth
        densdiff = idens - surfacedens  # get the daily density difference

        df = pd.DataFrame({"time": time, "densdiff": densdiff})
        monthlydiff = pd.DataFrame(
            df.resample("M", on="time").densdiff.mean()
        )  # average over months
...
def avg_eddy(eddy, time, ij, ii):
    ...
    with xr.open_dataset(
        "/data/vdo/MEOPAR/NEMO-forcing/grid/mesh_mask201702.nc"
    ) as mesh:
