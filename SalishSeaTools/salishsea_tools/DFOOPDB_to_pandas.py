...
def loadDFO(basedir, varlist={}, savedir="./"):
    ...
    if len(varlist) == 0:
        ...
        choosevars = {
            ...
            "Flag_Carbon_Dissolved_Organic",
            "Carbon_Particulate_Organic",
            "Carbon_Particulate_Total",
            "Flag_Carbon_Particulate_Total",
            "Flag_Chlorophyll",
            "Chlorophyll_Extracted",
            "Flag_Chlorophyll_Extracted",
            "Chlorophyll_Extracted_gt0point7um",
            "Chlorophyll_Extracted_gt5point0um",
            "Chlorophyll_plus_PhaeoPigment_Extracted",
            "Date",
            "Depth",
            "Depth_Nominal",
            "Flag_Salinity",
            "Flag_Salinity_Bottle",
            "Flag_Silicate",
            "Flag_pH",
            "Fluorescence_URU",
            "Fluorescence_URU_Seapoint",
            "Fluorescence_URU_Seatech",
            ...
            "Flag_Nitrate_plus_Nitrite",
            "Nitrate_plus_nitrite_ISUS",
            "Nitrate_plus_nitrite_ISUS_Voltage",
            "Nitrite",
            "Flag_Nitrite",
            "Nitrogen_Dissolved_Organic",
            "Flag_Nitrogen_Dissolved_Organic",
            "Nitrogen_Particulate_Organic",
            "Nitrogen_Particulate_Total",
            "Flag_Nitrogen_Particulate_Total",
            "Oxygen",
            "Quality_Flag_Oxyg",
            "Oxygen_Dissolved",
            "Flag_Oxygen_Dissolved",
            "Oxygen_Dissolved_SBE",
            "PAR",
            "PAR_Reference",
            "PhaeoPigment_Extracted",
            "Flag_PhaeoPigment_Extracted",
            "Flag_Phaeophytin",
            "Phosphate",
            "Flag_Phosphate",
            "Quality_Flag_Phos",
            "Phosphate(inorg)",
            "Phytoplankton_Volume",
            "Pressure",
            "Pressure_Reversing",
            "Production_Primary",
            "Quality_Flag_Nitr",
        ...
        chosevars = varlist
    varlistu = choosevars | {
        x + "_units" for x in choosevars if not re.search("Flag", x)
    }
    ...
    def coltype(ikey):
        ...
        return typedict.get(
            ikey, float
        )  # 2nd argument is default value returned if ikey not in typedict
    ...
    dfObs = pd.DataFrame(
        ...
        + list([cname for cname in varlistu])
    )
            ...
            for line in f:
                if infile:
                    if re.match("\s*\$", line) or len(line) == 0:
                        ...
                        if re.match("START TIME", splitline[0]):
                            ...
                            stationData["StartHour"] = (
                                float(splitTime[0])
                                + float(splitTime[1]) / 60.0
                                + float(splitTime[2]) / 3600.0
                            )
                ...
                if invars:
                    if re.search("\$END", line):
                        ...
                        test = re.findall(
                            "'.*?'", line
                        )  # (.*? matches anything but chooses min len match - not greedy)
                        for expr in test:
                            line = re.sub(
                                re.escape(expr), re.sub(" ", "_", expr), line
                            )  # remove spaces from items in quotes
                    ...
                    if re.search("\$END", line):
                        indetail = False
                    elif detcount == 1 and re.match(
                        "\s*\!\s*No\s*Pad\s*Start\s*Width", line
                    ):
                        detformat = True
                    else:
                    ...
                    if len(line) == 0:
                        ...
                        if re.match("STATION", splitline[0]):
                            ...
                            staLat = signdict[latparts[2]] * (
                                float(latparts[0]) + float(latparts[1]) / 60.0
                            )
                            ...
                            signdict = {"N": 1, "E": 1, "S": -1, "W": -1}
                            staLon = signdict[lonparts[2]] * (
                                float(lonparts[0]) + float(lonparts[1]) / 60.0
                            )
                    ...
                    if detformat:
                        ...
                        if len(splitline) == max(varNames.keys()):
                            ...
                            ferr.write(
                                "ERROR: filename:" + sourceFile + " line:" + line
                            )
                if re.match("![- ]*$", line):
                    tem = re.search("(?<=\!)[- ]*$", line)
