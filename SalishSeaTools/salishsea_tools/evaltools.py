        ...
        for la, lo in np.unique(data.loc[:, ["Lat", "Lon"]].values, axis=0):
            try:
                jj, ii = geo_tools.find_closest_model_point(
                    lo, la, navlon, navlat, grid=nemops, land_mask=lmask, checkTol=True
                )
            except:
                print("lo:", lo, "la:", la)
                raise
...
def _vertNetmatch(data, flist, ftypes, filemap_r, gridmask, e3t0, maskName="tmask"):
    ...
    for ind, row in data.iterrows():
        ...
        if ind == 0:  # special case for start of loop; load first files
            ...
            torig = dict()
            for ift in ftypes:
                fid, fend = _nextfile_bin(
                    ift, row["dtUTC"], flist[ift], fid, fend, flist
                )
                # handle NEMO files time reference
                if "time_centered" in fid[ftypes[0]].variables.keys():
                    torig[ift] = dt.datetime.strptime(
                        fid[ftypes[0]].variables["time_centered"].time_origin,
                        "%Y-%m-%d %H:%M:%S",
                    )
                else:
                    torig[ift] = dt.datetime.strptime(
                        fid[ftypes[0]].variables["time_counter"].time_origin,
                        "%Y-%m-%d %H:%M:%S",
                    )
        ...
        for ift in ftypes:
            ...
            try:
                if "time_centered_bounds" in fid[ift].variables.keys():  # no problem!
                    ...
                    hpf = (
                        flist[ift]["t_n"][0] - flist[ift]["t_0"][0]
                    ).total_seconds() / 3600  # hours per file
            ...
            if (
                len(
                    set(fid[ift].variables.keys()).intersection(
                        set(("deptht_bounds", "depthu_bounds", "depthv_bounds"))
                    )
                )
                > 0
            ):  # no problem!
            ...
            if (
                (not np.isnan(ik_l))
                and (not np.isnan(ik_u))
                and (gridmask[0, ik_u, row["j"], row["i"]] == 1)
            ):
                data.loc[ind, ["k_upper"]] = int(ik_u)
                data.loc[ind, ["k_lower"]] = int(ik_l)
                for ivar in filemap_r[ift]:
                    var = fid[ift].variables[ivar][
                        ih, ik_u : (ik_l + 1), row["j"], row["i"]
                    ]
                    ...
                    if gridmask[0, ik_l, row["j"], row["i"]] == 0:
                        print(
                            f"Warning: lower limit is not an ocean value:",
                ...
                print(
                    f"Warning: upper limit is not an ocean value:",
    ...
    for ind, row in data.iterrows():
        ...
        for ift in ftypes:
            ...
            if ift == "ops":  # special handling for ops atm forcing files
                ...
                try:
                    if (
                        "time_centered_bounds" in fid[ift].variables.keys()
                    ):  # no problem!
                        ih = _getTimeInd_bin(row["dtUTC"], fid[ift], torig[ift])
                    else:  # annoying!
                        hpf = (
                        ...
                        ih = _getTimeInd_bin(
                            row["dtUTC"], fid[ift], torig[ift], hpf=hpf
                        )
                except:
                    print("fend", fend)
            ...
            if sdim == 3:
                if preIndexed:
                    ...
                    if (not np.isnan(ik)) and (
                        gridmask[0, ik, row["j"], row["i"]] == 1
                    ):
                        for ivar in filemap_r[ift]:
                            try:
                                data.loc[ind, ["mod_" + ivar]] = fid[ift].variables[
                                    ivar
                                ][ih, ik, row["j"], row["i"]]
                            except:
                    ...
                    if (
                        ...
                        ik = _getZInd_bin(
                            row["Z"], fid[ift], boundsFlag=True, maskName=maskName
                        )
                    # assign values for each var assoc with ift
                    if (not np.isnan(ik)) and (
                        gridmask[0, ik, row["j"], row["i"]] == 1
...
def _vvlBin(data, flist, ftypes, filemap, filemap_r, tmask, fdict, e3tvar):
    ...
    data["indf"] = [
        int(
            flist[ifte3t]
            .loc[(aa >= flist[ifte3t].t_0) & (aa < flist[ifte3t].t_n)]
            .index[0]
        )
    ...
    data["ih"] = [
        int(np.floor((aa - bb).total_seconds() / (pere3t * 3600)))
            ...
            for (ih, jj, ii), grp1 in grp0.groupby(["ih", "j", "i"]):
                ...
                for ift in fdict_r[pere3t]:
                    for iz, iind in zip(ztar, grp1.index):
                        ik = [iii for iii, hhh in enumerate(zl) if hhh[1] > iz][
                            0
                        ...
                        if (not np.isnan(ik)) and (tmask[0, ik, jj, ii] == 1):
                            ...
                            for ivar in filemap_r[ift]:
                                data.loc[iind, ["mod_" + ivar]] = ff[ift].variables[
                                    ivar
                                ][ih, ik, jj, ii]
            for ift in fdict_r[pere3t]:
                ff[ift].close()
...
def _interpvvlZ(data, flist, ftypes, filemap, filemap_r, tmask, fdict, e3tvar):
    ...
    for indf, grp0 in data.groupby(["indf"]):
        with nc.Dataset(flist[ifte3t].loc[indf, ["paths"]].values[0]) as fe3t:
            ...
            for (ih, jj, ii), grp1 in grp0.groupby(["ih", "j", "i"]):
                e3t = fe3t.variables[e3tvar][ih, :, jj, ii][tmask[0, :, jj, ii] == 1]
                zs = np.cumsum(e3t) - 0.5 * e3t
                ztar = grp1["Z"].values
                for ift in fdict_r[pere3t]:
                    for ivar in filemap_r[ift]:
                        vals = ff[ift].variables[ivar][ih, :, jj, ii][
                            tmask[0, :, jj, ii] == 1
                        ]
                        data.loc[grp1.index, ["mod_" + ivar]] = np.where(
                            ztar < np.sum(e3t), np.interp(ztar, zs, vals), np.nan
                        )
            for ift in fdict_r[pere3t]:
                ff[ift].close()
    return data
...
def _ferrymatch(data, flist, ftypes, filemap_r, gridmask, fdict):
    ...
    for ift in ftypes:
        data["indf_" + ift] = [
            int(flist[ift].loc[(aa >= flist[ift].t_0) & (aa < flist[ift].t_n)].index[0])
            for aa in data["dtUTC"]
        ]
        t2 = [
        ...
        for ind, row in data.iterrows():
            ...
            if not row["indf_" + ift] == indflast:
                if not indflast == -1:
                    fid.close()
                fid = nc.Dataset(
                    flist[ift].loc[row["indf_" + ift], ["paths"]].values[0]
                )
                indflast = row["indf_" + ift]
            for ivar in filemap_r[ift]:
                data.loc[ind, ["mod_" + ivar]] = fid.variables[ivar][
                    row["ih_" + ift], 0, row["j"], row["i"]
                ]
    return data


def _nextfile_bin(
    ift, idt, ifind, fid, fend, flist
):  # to do: replace flist[ift] with ifind and get rid of flist argument
...
def _getTimeInd_bin(idt, ifid, torig, hpf=None):
    ...
    if "time_centered_bounds" in ifid.variables.keys():
        ...
        tlist = [
            ii * tcorr + hpf / (nt * 2) * 3600
            for ii in ifid.variables["time_counter"][:]
        ]
        ih = [
            iii for iii, hhh in enumerate(tlist) if hhh > (idt - torig).total_seconds()
        ][0]
    return ih
...
def _getTimeInd_bin_ops(idt, ifid, torig):
    ...
    ih = [
        iii
        for iii, hhh in enumerate(tlist)
        if (hhh + tinterval) > (idt - torig).total_seconds()
    ][0]
    return ih
...
def _getZInd_bin(idt, ifid=None, boundsFlag=False, maskName="tmask"):
    ...
    if idt <= np.max(tlist):
        ih = [iii for iii, hhh in enumerate(tlist) if hhh[1] > idt][
            0
        ]  # return first index where latter endpoint is larger
    else:
        ih = np.nan
...
def index_model_files(start, end, basedir, nam_fmt, flen, ftype=None, tres=1):
    ...
    while True:
        try:
            ipathstr = os.path.join(
                basedir,
                stencil.format(
                    iits.strftime(dfmt).lower(),
                    iits.strftime(ffmt),
                    iite.strftime(ffmt),
                    iits.strftime(wfmt),
                ),
    ...
    while iits < end:
        ...
        ind = ind + 1
    return pd.DataFrame(
        data=np.swapaxes([paths, t_0, t_n], 0, 1),
        index=inds,
        columns=["paths", "t_0", "t_n"],
    )


def index_model_files_flex(
    basedir, ftype, freq="1d", nam_fmt="nowcast", start=None, end=None
):
    """
    See inputs for matchData above.
    outputs pandas dataframe containing columns 'paths','t_0', and 't_1'
    ...
    """
    paths = glob.glob(
        os.path.join(basedir, "???????", "*" + ftype + "*")
    )  # assume if there are subdirectories, they must have nowcast yymmmdd format
    if len(paths) == 0:  # in case of no subdirectories
        paths = glob.glob(os.path.join(basedir, "*" + ftype + "*"))
    paths = [
        el for el in paths if re.search(freq, el)
    ]  # restrict to files with desired output frequency
    t_0 = list()
    t_n = list()
    for ifl in paths:
        if nam_fmt == "nowcast":
...
def loadDFOCTD(
    ...
    try:
        ...
        raise ImportError(
            "You need to install sqlalchemy in your environment to use this function."
        )
    ...
    SA = case(
        ...
        else_=case(
            ...
            else_=case(
                [(CalcsTBL.Salinity_SA != None, CalcsTBL.Salinity_SA)], else_=None
            ),
        ),
    )
    CT = case(
    ...
    FL = case(
        [(ObsTBL.Fluorescence_URU_Seapoint != None, ObsTBL.Fluorescence_URU_Seapoint)],
    ...
    if len(datelims) < 2:
        qry = (
            session.query(
                ...
                CT.label("CT"),
                FL.label("Fluor"),
                ObsTBL.Oxygen_Dissolved_SBE.label("DO_mLL"),
                ObsTBL.Oxygen_Dissolved_SBE_1.label("DO_umolkg"),
            )
            .select_from(StationTBL)
        ...
        qry = (
            ...
            .filter(
                and_(
                    ...
                    or_(
                        StationTBL.StartYear < end_y,
                        and_(
                            StationTBL.StartYear == end_y, StationTBL.StartMonth < end_m
                        ),
                        and_(
                            StationTBL.StartYear == end_y,
                            StationTBL.StartMonth == end_m,
                            StationTBL.StartDay < end_d,
                        ),
                    ),
                    StationTBL.Lat > 47 - 3 / 2.5 * (StationTBL.Lon + 123.5),
                    StationTBL.Lat < 47 - 3 / 2.5 * (StationTBL.Lon + 121),
    ...
    df1["dtUTC"] = [
        dt.datetime(int(y), int(m), int(d)) + dt.timedelta(hours=h)
...
def loadDFO(
    ...
    if excludeSaanich:
        qry1 = qry.filter(
            not_(
                and_(
                    StationTBL.Lat > 48.47,
                    StationTBL.Lat < 48.67,
                    StationTBL.Lon > -123.6,
                    StationTBL.Lon < -123.43,
                )
            )
        )
    ...
    df1["Z"] = np.where(
        df1["Depth"] >= 0,
        df1["Depth"],
        -1.0 * gsw.z_from_p(p=df1["Pressure"].values, lat=df1["Lat"].values),
    )
    df1["dtUTC"] = [
        dt.datetime(int(y), int(m), int(d)) + dt.timedelta(hours=h)
        for ind, (y, m, d, h) in df1.loc[:, ["Year", "Month", "Day", "Hour"]].iterrows()
    ]
    session.close()
    engine.dispose()
    return df1
...
def loadPSF(datelims=(), loadChl=True, loadCTD=False):
    ...
    if datelims[0].year < 2016:
        ...
        f2015 = f2015.dropna(
            subset=["date (dd/mm/yyyy)", "Time (Local)", "lat", "lon", "depth"],
            how="any",
        )
        ...
        dts = [
            ...
            .localize(
                dt.datetime.strptime(ii, "%Y-%m-%d %H:%M:%S")
                + dt.timedelta(hours=jj.hour, minutes=jj.minute, seconds=jj.second)
            )
            .astimezone(pytz.utc)
            .replace(tzinfo=None)
        ...
        if loadChl:
            # load 2015 chl
            Chl2015 = pd.read_csv(
                "/ocean/eolson/MEOPAR/obs/PSFCitSci/Chla_2015PSFSalish_Sea_22.01.2018vers_8_CN_edits.csv",
                encoding="latin-1",
                dtype={
                    "Date sampled (mm/dd/yyyy)": str,
                    "Time of Day (Local)": str,
                    "Latitude": str,
                    "Longitude": str,
                    "Chl a": float,
                    "Phaeophytin": float,
            ...
            Chl2015["Lon"] = [
                -1.0 * (float(ii[0]) + float(ii[1]) / 60) for ii in degminlon
            ]
            Chl2015 = Chl2015.dropna(
                subset=[
            ...
            dts = [
                pytz.timezone("Canada/Pacific")
                .localize(dt.datetime.strptime(ii + "T" + jj, "%m/%d/%yT%I:%M:%S %p"))
                .astimezone(pytz.utc)
                .replace(tzinfo=None)
                for ii, jj in zip(ds, ts)
            ]
            ...
            Chl2015_g = Chl2015.groupby(
                ["Station", "Lat", "Lon", "dtUTC", "Z"], as_index=False
            )
            Chl2015_m = Chl2015_g.mean()
            Chl2015 = Chl2015_m.reindex()
            dfchls.append(Chl2015)
        if loadCTD:
            phys2015 = pd.read_csv(
                "/ocean/eolson/MEOPAR/obs/PSFCitSci/phys/CitSci2015_20180621.csv",
                ...
                converters={
                    "pressure": lambda x: float(x),
                    "depth": lambda x: float(x),
                    "temperature": lambda x: float(x),
                    "conductivity": lambda x: float(x),
                    "salinity": lambda x: float(x),
                    "o2SAT": lambda x: float(x),
                    "o2uM": lambda x: float(x),
                    "chl": lambda x: float(x),
                },
            )
            ctddfs[2015] = dict()
    ...
    if (datelims[0].year < 2017) and (datelims[1].year > 2015):
        ...
        dts = [
            pytz.timezone("Canada/Pacific")
            .localize(
                dt.datetime(ii.year, ii.month, ii.day)
                + dt.timedelta(hours=jj.hour, minutes=jj.minute, seconds=jj.second)
            )
            .astimezone(pytz.utc)
            .replace(tzinfo=None)
        ...
        f2016Si = f2016Si.dropna(
            subset=["DDMMYYYY", "Time (Local)", "Latitude", "Longitude", "Depth"],
            how="any",
        )
        ds = f2016Si["DDMMYYYY"]
        ...
        f2016Si.drop(
            ["DDMMYYYY", "Time (Local)", "Lat_reported", "Long_reported", "Depth"],
            axis=1,
            inplace=True,
        )
        f2016Si_g = f2016Si.groupby(
            ["Station", "Lat", "Lon", "dtUTC", "Z"], as_index=False
        )
        ...
        f2016 = pd.merge(
            ...
            f2016Si_m,
            how="outer",
            left_on=["Station", "Lat", "Lon", "dtUTC", "Z"],
            right_on=["Station", "Lat", "Lon", "dtUTC", "Z"],
        )
        dfs.append(f2016)
        if loadChl:
            # load 2016 chl
            Chl2016Dat = pd.read_csv(
                "/ocean/eolson/MEOPAR/obs/PSFCitSci/2016ChlorophyllChlData.csv"
            )  # ,encoding='latin-1')
            Chl2016Sta = pd.read_csv(
                "/ocean/eolson/MEOPAR/obs/PSFCitSci/2016ChlorophyllStationData.csv"
            )
            Chl2016Sta.rename(
                columns={
            ...
            Chl2016Sta.dropna(
                subset=["DateCollected", "TimeCollected", "Lat", "Lon", "Depth_m"],
                how="any",
                inplace=True,
            )
            Chl2016Sta.drop_duplicates(inplace=True)
            Chl2016Dat.drop(
                Chl2016Dat.loc[Chl2016Dat.quality_flag > 3].index, axis=0, inplace=True
            )
            Chl2016Dat.drop(
                ["Chla_ugL", "Phaeophytin_ugL", "quality_flag", "ShipBoat"],
                axis=1,
                inplace=True,
            )
            Chl2016Dat.rename(
                columns={"MeanChla_ugL": "Chl", "MeanPhaeophytin_ugL": "Phaeo"},
                inplace=True,
            )
            Chl2016 = pd.merge(
                Chl2016Sta,
                Chl2016Dat,
                how="inner",
                left_on=["DateCollected", "Station", "Depth_m"],
                right_on=["DateCollected", "Station", "Depth_m"],
            )
        ...
        if loadCTD:
            phys2016 = pd.read_csv(
                "/ocean/eolson/MEOPAR/obs/PSFCitSci/phys/CitSci2016_20180621.csv",
                skiprows=lambda x: x in [0, 1, 2, 3, 4, 5, 6, 7, 9],
                delimiter=",",
                dtype={
                    "Patrol": str,
                    "ID": str,
    ...
    if datelims[1].year > 2016:
        ...
        f2017 = pd.read_excel(
            "/ocean/eolson/MEOPAR/obs/PSFCitSci/All_Yrs_Nutrients_2018-01-31_EOEdit.xlsx",
            sheet_name="2017 N+P+Si",
            skiprows=3,
            dtype={
                "Date (dd/mm/yyyy)": dt.date,
                "Time (Local)": dt.time,
                "NO3+NO": str,
                "PO4": str,
                "Si": str,
        ...
        if loadChl:
            # load 2017 chl
            Chl2017 = pd.read_excel(
                "/ocean/eolson/MEOPAR/obs/PSFCitSci/PSF 2017 Chla_Data_Final_v-January 22-2018_CN_edits.xlsx",
                sheet_name="avg-mean-cv%",
                skiprows=15,
        ...
        if loadCTD:
            phys2017 = pd.read_csv(
                "/ocean/eolson/MEOPAR/obs/PSFCitSci/phys/CitSci2017_20180621.csv",
    ...
    if loadCTD:
        ...
        for ik in ctddfs.keys():
            idf = ctddfs[ik]["df"]
            dtsP = [
                dt.datetime.strptime(ii, "%d/%m/%Y %H:%M:%S")
                for ii in idf["datetime"].values
            ]
            ...
            idf["HrsPast"] = [
                (ii - dt.datetime(2014, 1, 1)).total_seconds() / 3600 for ii in dtsP
            ]
            idf["CT"] = [
                gsw.CT_from_t(SA, t, p)
                for SA, t, p, in zip(
                    idf["salinity"], idf["temperature"], idf["pressure"]
                )
            ]
        ...
        df1["HrsPast"] = [
            (ii - dt.datetime(2014, 1, 1)).total_seconds() / 3600 for ii in df["dtUTC"]
        ]
        df["SA"] = np.nan
        df["CT"] = np.nan
        df["pLat"] = np.nan
        ...
        for ik in ctddfs.keys():
            ...
            for i, row in df1.loc[
                (df1["dtUTC"] > dtlims[0]) & (df1["dtUTC"] < dtlims[1])
            ].iterrows():
                idf = jdf.loc[
                    (jdf.StaTrim == row["StaTrim"])
                    & (jdf.depth == np.round(row["Z"]))
                    & (np.abs(jdf.HrsPast - row["HrsPast"]) < 1.0)
                    & (np.abs(jdf.latitude - row["Lat"]) < 0.05)
                    & (np.abs(jdf.longitude - row["Lon"]) < 0.05)
                ]
                if len(idf) > 0:
                    ...
                    tdelta = np.nanmean(
                        [
                            np.abs((ii - row["dtUTC"]).total_seconds() / 3600)
                            for ii in idf["dtUTCPhys"]
                        ]
                    )
                    df.at[i, "SA"] = sal
                    df.at[i, "CT"] = tem
...
def loadPSFCTD(datelims=(), pathbase="/ocean/eolson/MEOPAR/obs/PSFCitSci/phys"):
    ...
    if datelims[0].year < 2016:
        # load 2015
        phys2015 = pd.read_csv(
            os.path.join(pathbase, "CitSci2015_20180621.csv"),
            skiprows=lambda x: x in [0, 1, 2, 3, 4, 6],
            delimiter=",",
            dtype={
                "Patrol": str,
    ...
    if (datelims[0].year < 2017) and (datelims[1].year > 2015):
        # load 2016
        phys2016 = pd.read_csv(
            os.path.join(pathbase, "CitSci2016_20180621.csv"),
            skiprows=lambda x: x in [0, 1, 2, 3, 4, 5, 6, 7, 9],
            delimiter=",",
            dtype={
                "Patrol": str,
    ...
    if datelims[1].year > 2016:
        # load 2017
        phys2017 = pd.read_csv(
            os.path.join(pathbase, "CitSci2017_20180621.csv"),
            skiprows=lambda x: x in [0, 1, 2, 3, 4, 5, 7],
            delimiter=",",
            dtype={
                "Patrol": str,
    ...
    df["dtUTC"] = [
        dt.datetime.strptime(ii, "%d/%m/%Y %H:%M:%S") for ii in df["datetime"].values
    ]
    df["CT"] = [
        gsw.CT_from_t(SA, t, p)
        for SA, t, p, in zip(df["salinity"], df["temperature"], df["pressure"])
    ]
    df.rename(
        columns={"salinity": "SA", "latitude": "Lat", "longitude": "Lon", "depth": "Z"},
        inplace=True,
    )
    return df
...
def loadHakai(datelims=(), loadCTD=False):
    ...
    fc = pd.read_csv(
        "/ocean/eolson/MEOPAR/obs/Hakai/Dosser20180911/ctd-bulk-1536702711696.csv",
        usecols=[
            "Cast PK",
            "Cruise",
            "Station",
        ...
        dtype={
            "Drop number": np.float64,
            "PAR flag": str,
            "Fluorometry Chlorophyll flag": str,
        },
        na_values=("null", "-9.99e-29"),
    )
    ...
    fc["dt"] = [
        dt.datetime.strptime(i.split(".")[0], "%Y-%m-%d %H:%M:%S")
        for i in fc["Start time"]
    ]
    dts = [
        pytz.timezone("Canada/Pacific")
        .localize(dt.datetime.strptime(i.split(".")[0], "%Y-%m-%d %H:%M:%S"))
        .astimezone(pytz.utc)
        .replace(tzinfo=None)
        for i in fc["Start time"]
    ]
    ...
    fcS = (
        fc.loc[:, ["Latitude", "Longitude"]]
        .groupby([fc["Station"], fc["dloc"]])
        .mean()
        .reset_index()
    )
    ...
    zthresh = 1.5
    print(
        "Note: CTD depths (pZ) may vary from bottle depths (Z) by up to ",
        str(zthresh),
        " m.",
    )
    for i, row in df2.iterrows():
        idf = fc.loc[
            (fc.Station == row["Station"])
            & (fc.dloc == row["dloc"])
            & (
                (
                    np.abs(fc["Depth (m)"] - row["Pressure Transducer Depth (m)"])
                    < zthresh
                )
                | (np.abs(fc["Depth (m)"] - row["Line Out Depth"]) < zthresh)
            )
        ]
        if len(idf) > 0:
            zrow = (
                row["Pressure Transducer Depth (m)"]
                if ~np.isnan(row["Pressure Transducer Depth (m)"])
                else row["Line Out Depth"]
            )
            ...
            isna = (
                (not np.isnan(idfZ["Salinity (PSU)"].values[0]))
                and (not np.isnan(idfZ["Pressure (dbar)"].values[0]))
                and (not np.isnan(idfZ["Longitude"].values[0]))
                and (not np.isnan(idfZ["Latitude"].values[0]))
            )
            isnat = not np.isnan(idfZ["Temperature (deg C)"].values[0])
            sal = (
                gsw.SA_from_SP(
                    idfZ["Salinity (PSU)"].values[0],
                    idfZ["Pressure (dbar)"].values[0],
                    idfZ["Longitude"].values[0],
                    idfZ["Latitude"].values[0],
                )
                if isna
                else np.nan
            )
            tem = (
                gsw.CT_from_t(
                    sal,
                    idfZ["Temperature (deg C)"].values[0],
                    idfZ["Pressure (dbar)"].values[0],
                )
                if (isna and isnat)
                else np.nan
    ...
    fdata2 = (
        fdata.loc[
            (fdata["dtUTC"] >= start_date)
            & (fdata["dtUTC"] < end_date)
            & (fdata["Z"] >= 0)
            & (fdata["Z"] < 440)
            & (fdata["Lon"] < 360)
            & (fdata["Lat"] <= 90)
...
def WSS(obs, mod):
    ...
    return 1.0 - np.sum((mod - obs) ** 2) / np.sum(
    ...
    if len(sepvals) == 0:
        ...
        if np.sum(iii) > 0:
            ...
            if len(labels) > 0:
                ...
                ll = "{} $<$ {} {}".format(lname, sepvals[ii], sepunits).strip()
...
def tsertser_graph(
    ...
    mcols=("fuchsia", "firebrick", "orange", "darkgoldenrod", "maroon"),
    labels="",
):
    ...
    if len(sepvals) == 0:
        obs0 = _deframe(
            df.loc[(df["dtUTC"] >= start_date) & (df["dtUTC"] <= end_date), [obsvar]]
        )
        mod0 = _deframe(
            df.loc[(df["dtUTC"] >= start_date) & (df["dtUTC"] <= end_date), [modvar]]
        )
        time0 = _deframe(
            df.loc[(df["dtUTC"] >= start_date) & (df["dtUTC"] <= end_date), ["dtUTC"]]
        )
        ...
        if np.sum(iii) > 0:
            ...
            if len(labels) > 0:
                ...
                ll = "{} $<$ {} {}".format(lname, sepvals[ii], sepunits).strip()
            (p0,) = ax.plot(
                time0[iii], obs0[iii], ".", color=ocols[ii], label=f"Observed {ll}"
            )
            ps.append(p0)
            (p0,) = ax.plot(
                time0[iii], mod0[iii], ".", color=mcols[ii], label=f"Modeled {ll}"
            )
            ps.append(p0)
        # between min and max:
        for ii in range(1, len(sepvals)):
        ...
        if np.sum(iii) > 0:
            ...
            ps.append(p0)
            (p0,) = ax.plot(
                time0[iii], mod0[iii], ".", color=mcols[ii + 1], label=f"Modeled {ll}"
            )
            ps.append(p0)
...
def displayStats(statdict, level="Subset", suborder=None):
    ...
    pcols = {"Subset": ["Metric"], "Variable": ["Metric"], "Year": ["Year", "Metric"]}
    ...
    tbl = (
        pd.pivot_table(tdf, index=ind[level][:-1], columns=pcols[level])
        .rename_axis(index={"Order": None}, columns={"Metric": None})
        .style.format(
            {"N": "{:d}", "Bias": "{:.3f}", "WSS": "{:.3f}", "RMSE": "{:.3f}"}
        )
    )
    return tbl, tdf
...
def displayStatsFlex(statdict, cols, ind, pcols, suborder=None):
    ...
    tbl = (
        pd.pivot_table(tdf, index=ind[:-1], columns=pcols)
        .rename_axis(index={"Order": None}, columns={"Metric": None})
        .style.format(
            {"N": "{:d}", "Bias": "{:.3f}", "WSS": "{:.3f}", "RMSE": "{:.3f}"}
        )
...
def utc_to_pac(timeArray):
    ...
    return [
        pytz.utc.localize(ii).astimezone(pytz.timezone("Canada/Pacific"))
        for ii in timeArray
    ]
...
def printstats(datadf, obsvar, modvar):
    N, modmean, obsmean, bias, RMSE, WSS = stats(
        datadf.loc[:, [obsvar]], datadf.loc[:, [modvar]]
    )
...
def load_Pheo_data(year, datadir="/ocean/eolson/MEOPAR/obs/WADE/ptools_data/ecology"):
    ...
    dfTime = pd.read_excel(
        "/ocean/eolson/MEOPAR/obs/WADE/WDE_Data/OlsonSuchyAllen_UBC_PDR_P003790-010721.xlsx",
        engine="openpyxl",
        sheet_name="EventDateTime",
    )
    test = dfTime.groupby(["FlightDate", "SiteCode"])[
        "TimeDown \n(Local - PST or PDT)"
    ].count()
    # drop duplicate rows
    dfTime.drop_duplicates(
        subset=["FlightDate", "SiteCode"], keep="first", inplace=True
    )
    print(dfTime.keys())
    dfTime["dtPac"] = [
        dt.datetime.combine(idate, itime)
        for idate, itime in zip(
            dfTime["FlightDate"], dfTime["TimeDown \n(Local - PST or PDT)"]
        )
    ]
    ...
    sta_df.dropna(
        how="any",
        subset=["Lat_NAD83 (deg / dec_min)", "Long_NAD83 (deg / dec_min)", "Station"],
        inplace=True,
    )
    ...
    chlPheo2 = pd.DataFrame(
        chlPheo.groupby(["Date", "Station", "SamplingDepth"], as_index=False).mean()
    )
    # join to station info (lat/lon)
    chlPheo3 = pd.merge(
        left=sta_df, right=chlPheo2, how="right", left_on="Station", right_on="Station"
    )
    # join to date/time
    dfTime["dtUTC"] = [
        pac_to_utc(dt.datetime.combine(idate, itime))
        for idate, itime in zip(
            dfTime["FlightDate"], dfTime["TimeDown \n(Local - PST or PDT)"]
        )
    ]
    dfTime2 = dfTime.loc[:, ["FlightDate", "SiteCode", "dtUTC"]]
    chlPheoFinal = pd.merge(
        left=chlPheo3,
        right=dfTime2,
        how="left",
        left_on=["Date", "Station"],
        right_on=["FlightDate", "SiteCode"],
    )
...
def load_WADE_data(year, datadir="/ocean/eolson/MEOPAR/obs/WADE/ptools_data/ecology"):
    ...
    df = pd.merge(
        left=dfSta, right=dfBot, how="right", left_on="Station", right_on="Station"
    )
    try:
    ...
    try:
        ...
        pass
        print(
            f"Warning!, Merge completed incorrectly. length of bottle data = {len(dfBot)} length of merged data = {len(df)}"
        )
    # where no time is provided, set time to midday Pacific time = ~ 20:00 UTC
    df["UTCDateTime"] = [
        iiD + dt.timedelta(hours=20) if pd.isnull(iiU) else iiU
        for iiU, iiD in zip(df["UTCDateTime"], df["Date"])
    ]
    df.rename(
        columns={"UTCDateTime": "dtUTC", "Latitude": "Lat", "Longitude": "Lon"},
        inplace=True,
    )
    df["Z"] = -1 * df["Z"]
    df.head()
    df["NO23"] = (
        df["NO3(uM)D"] + df["NO2(uM)D"]
    )  # the model does not distinguish between NO2 and NO3
    df["Amm"] = df["NH4(uM)D"]
    df["Si"] = df["SiOH4(uM)D"]
...
def load_CTD_data(year, datadir="/ocean/eolson/MEOPAR/obs/WADE/ptools_data/ecology"):
    ...
    dfCTD = pd.merge(
        left=dfSta, right=dfCTD0, how="right", left_on="Station", right_on="Station"
    )
    try:
    ...
    # where no time is provided, set time to midday Pacific time = ~ 20:00 UTC
    dfCTD["dtUTC"] = [
        iiD + dt.timedelta(hours=20) for iiD in dfCTD["Date"]
    ]  # Does this mean it also has that flaw where we are not sure when the data was collected?
    ...
    dfCTD["SA"] = gsw.SA_from_SP(dfCTD["Salinity"], press, dfCTD["Lon"], dfCTD["Lat"])
