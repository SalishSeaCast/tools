...
def DispGridLocs(
    mesh_mask="/ocean/eolson/MEOPAR/NEMO-forcing/grid/mesh_mask201702_noLPE.nc",
):
    ...
    ax[0].pcolormesh(
        np.ma.masked_where(tmask[0, 0, :, :] == 0, bathy), cmap=plt.cm.get_cmap("Blues")
    )
    # map stations:
    for pl in places2.keys():
        if (
            "NEMO grid ji" in places2[pl].keys()
            and places2[pl]["NEMO grid ji"] is not None
        ):
