...
def mean_tracer_at_depth(grid_t, tracer_name, min_depth=150, max_depth=None):
    ...
    tracer_quantity_array = (grid_t[tracer_name].values) * (
        grid_heights.reshape((1, 40, 1, 1))
    )
    total_tracer_at_depth = tracer_quantity_array[
        min_day_index:max_day_index, min_depth_index:max_depth_index, :, :
    ].sum()
    mean_tracer = (
        total_tracer_at_depth
        / sum(grid_heights[min_depth_index:max_depth_index])
        / (max_day_index - min_day_index)
    )
...
def time_of_peak_PHY2(grid_t):
    ...
    phy2_quantity_array = (grid_t["PHY2"].values) * (
        grid_heights.reshape((1, 40, 1, 1))
    )
...
def time_surface_NO3_drops_below_4(grid_t):
    ...
    mean_surface_NO3 = np.sum(
        (
            (grid_t.variables["NO3"][:, :, 1, 1] * grid_heights.reshape((1, 40)))[
                :, :10
            ]
            / sum(grid_heights[:10])
        ),
...
def peak_3_day_biomass(grid_t):
    ...
    for tracer in primary_producers:
        depth_integrated = depth_integrated + np.sum(
            (grid_t[tracer].values) * reshaped_grid_heights, axis=(1, 2, 3)
        )
