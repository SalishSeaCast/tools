...
def load_nml_bio(
    resDir,
    nmlname,
    bioRefName="namelist_smelt_ref",
    bioCfgName="namelist_smelt_cfg",
    namRefDir=None,
):
...
def each_limiter(
    ...
    # Light
    zz_plank_growth_light = (
        (1.0 - np.exp(-zz_I_par / (0.33 * zz_rate_Iopt)))
        * (np.exp(-zz_I_par / (30.0 * zz_rate_Iopt)))
        * 1.06
    )
    ...
    # Si
    zz_Sc = np.where(
        np.logical_and(zz_Si > 0.0, tmask > 0), zz_Si / (zz_rate_K_Si + zz_Si), 0.0
    )
    SiLim = zz_Sc
    # Nitrate and Ammonium
    zz_Oup_cell = np.where(
        np.logical_and(zz_NO > 0.0, tmask > 0),
        zz_NO * zz_rate_kapa / (zz_rate_k + zz_NO * zz_rate_kapa + zz_NH),
        0.0,
    )
    zz_Hup_cell = np.where(
        np.logical_and(zz_NH > 0.0, tmask > 0),
        zz_NH / (zz_rate_k + zz_NO * zz_rate_kapa + zz_NH),
        0.0,
    )
    if np.any(zz_Oup_cell < 0.0):
        raise ValueError("zz_Oup_cell<0")
    if np.any(zz_Hup_cell < 0.0):
        raise ValueError("zz_Hup_cell<0")
    NLim = zz_Oup_cell + zz_Hup_cell
    # set flags
    limiter = -1 * np.ones(zz_Si.shape)
    limiter = np.where(
        np.logical_and(ILim <= NLim, ILim <= SiLim),
        0,
...
def calc_nutLim_2(zz_NO, zz_NH, zz_Si, zz_rate_K_Si, zz_rate_kapa, zz_rate_k):
    ...
    zz_Hup_cell = np.where(
        zz_NH > 0.0, zz_NH / (zz_rate_k + zz_NO * zz_rate_kapa + zz_NH), 0.0
    )
    if np.any(zz_Oup_cell < 0.0):
        raise ValueError("zz_Oup_cell<0")
    if np.any(zz_Hup_cell < 0.0):
...
def calc_diat_sink(zz_w_sink_Pmicro_min, zz_w_sink_Pmicro_max, diatNutLim):
    ...
    wsink = zz_w_sink_Pmicro_min * diatNutLim + zz_w_sink_Pmicro_max * (
        1.0 - diatNutLim
    )
    return wsink / (
        24 * 3600
    )  # diatom sinking rates are converted to m/s during namelist read
...
def calc_p_limiters(I, NO, NH, Si, tmask, nampisprod):
    ...
    :arg nampisprod: namelist dict loaded using load_nml_bio with
                     argument nampisprod
    """
    ILimDiat, NLimDiat, SiLimDiat, limiterDiat, limvalDiat = each_limiter(
        I,
        NO,
        NH,
        Si,
        tmask,
        nampisprod["zz_rate_Iopt_diat"],
        nampisprod["zz_rate_gamma_diat"],
        nampisprod["zz_rate_k_Si_diat"],
        nampisprod["zz_rate_kapa_diat"],
        nampisprod["zz_rate_k_diat"],
    )

    ILimMyri, NLimMyri, SiLimMyri, limiterMyri, limvalMyri = each_limiter(
        I,
        NO,
        NH,
        Si,
        tmask,
        nampisprod["zz_rate_Iopt_myri"],
        nampisprod["zz_rate_gamma_myri"],
        nampisprod["zz_rate_k_Si_myri"],
        nampisprod["zz_rate_kapa_myri"],
        nampisprod["zz_rate_k_myri"],
    )

    ILimNano, NLimNano, SiLimNano, limiterNano, limvalNano = each_limiter(
        I,
        NO,
        NH,
        Si,
        tmask,
        nampisprod["zz_rate_Iopt_nano"],
        nampisprod["zz_rate_gamma_nano"],
        nampisprod["zz_rate_k_Si_nano"],
        nampisprod["zz_rate_kapa_nano"],
        nampisprod["zz_rate_k_nano"],
    )
    Diat = {
        "ILim": ILimDiat,
        "NLim": NLimDiat,
...
def phyto_Tdep_Factor(TT, zz_rate_maxtemp, zz_rate_temprange):
    ...
    return (
        np.exp(0.07 * (TT - 20))
        * np.minimum(np.maximum((zz_rate_maxtemp - TT), 0.0), zz_rate_temprange)
        / (zz_rate_temprange + 1e-10)
    )


def calc_T_Factors(TT, nampisprod):
    Tdep_Diat = phyto_Tdep_Factor(
        TT, nampisprod["zz_rate_maxtemp_diat"], nampisprod["zz_rate_temprange_diat"]
    )
    Tdep_Myri = phyto_Tdep_Factor(
        TT, nampisprod["zz_rate_maxtemp_myri"], nampisprod["zz_rate_temprange_myri"]
    )
    Tdep_Nano = phyto_Tdep_Factor(
        TT, nampisprod["zz_rate_maxtemp_nano"], nampisprod["zz_rate_temprange_nano"]
    )
