...
"""Prepare for and run the Salish Sea NEMO model for the second forecast (day after tomorrow) on Salish
...
def main():
    ...
    run_id = '{dmy}forecast2'.format(dmy=dmy)
    print tomorrow, 'Tomorrow'
    run_desc = run_description(run_id, tomorrow, nowcast_end)
    results_dir = os.path.join('../SalishSea/forecast2', dmy)
...
def update_time_namelist(runday, timesteps_per_day=TIMESTEPS_PER_DAY):
    ...
    threeph_day = timedelta(days=3.5)
    if next_itend / timesteps_per_day > (runday - date0 + threeph_day).days:
